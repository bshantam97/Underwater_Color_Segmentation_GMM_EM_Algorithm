...
class roipoly:
    ...
    def get_points(self, currentImage):
        ny, nx, _ = np.shape(currentImage)
        poly_verts = [(int(round(self.allxpoints[0])), int(round(self.allypoints[0])))]

        for i in range(len(self.allxpoints)-1, -1, -1):
            poly_verts.append((int(round(self.allxpoints[i])), int(round(self.allypoints[i]))))
        return poly_verts
